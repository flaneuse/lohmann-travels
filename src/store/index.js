  ...
  actions: {
    fetchData(context) {
      ...
      const readerOptions = {
        ...
        sheetId: '10USSu4hV24oyo32ynsafYNBwd2Vui_DNF8llKlS0V8Y', // Link to Google sheet. Should have permissions set to "anyone with link can view"
      ...
      reader(readerOptions, (results) => {
        ...
        context.state.geojson["features"].forEach(d => {
          ...
          if (filtered.length == 1) {
            ...
            const Shelley = filtered[0].Shelley === "1";
            const Jim = filtered[0].Jim === "1";

            // calculating the color
            const value = Shelley && Jim ? "kb" :
              Shelley ? "Shelley" :
              Jim ? "Jim" :
              "unknown";
            d["properties"]["Shelley"] = Shelley;
            d["properties"]["Jim"] = Jim;
